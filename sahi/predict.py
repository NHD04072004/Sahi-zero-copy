...
from sahi.slicing import slice_image, slice_image_gpu
...
def get_prediction_gpu(
    ...
    Args:
        image: str or np.ndarray or torch.Tensor
            Location of image or numpy image matrix to slice or torch.Tensor
        detection_model: model.DetectionMode
        shift_amount: List
            To shift the box and mask predictions from sliced image to full
            sized image, should be in the form of [shift_x, shift_y]
    ...
    image_as_tensor = read_image_as_tensor(image, device=device)
    # get prediction
    # ensure shift_amount is a list instance (avoid mutable default arg)
    if shift_amount is None:
        shift_amount = [0, 0]
    ...
    time_start = time.perf_counter()
    detection_model.perform_inference_gpu(image_as_tensor)
    time_end = time.perf_counter() - time_start
    durations_in_seconds["prediction"] = time_end
...
def get_sliced_prediction_gpu(
    ...
    Args:
        image: str or torch.Tensor
            Location of image or torch.Tensor image matrix to slice
        detection_model: model.DetectionModel
        slice_height: int
            Height of each slice.  Defaults to ``None``.
        slice_width: int
