...
from sahi.utils.cv import IMAGE_EXTENSIONS_LOSSLESS, IMAGE_EXTENSIONS_LOSSY, read_image_as_pil, read_image_as_tensor
...
def slice_image_gpu(
    ...
    Args:
        image (str or torch.Tensor): File path of image or torch.Tensor image matrix to be sliced.
        coco_annotation_list (List[CocoAnnotation], optional): List of CocoAnnotation objects.
        output_file_name (str, optional): Root name of output files (coordinates will
            be appended to this)
        output_dir (str, optional): Output directory
    ...
    def _export_single_slice(image: torch.Tensor, output_dir: str, slice_file_name: str):
        slice_file_path = str(Path(output_dir) / slice_file_name)
        # export sliced image
        cv2.imwrite(slice_file_path, image.cpu().numpy())
        verboselog("sliced image path: " + slice_file_path)
    ...
    for slice_bbox in slice_bboxes:
        ...
        if out_ext:
            suffix = out_ext
        elif hasattr(image_tensor, "filename"):
            suffix = Path(getattr(image_tensor, "filename")).suffix
            if suffix in IMAGE_EXTENSIONS_LOSSY:
                suffix = ".png"
            elif suffix in IMAGE_EXTENSIONS_LOSSLESS:
                suffix = Path(image_tensor.filename).suffix
        else:
            suffix = ".png"
        ...
        # create sliced image and append to sliced_image_result
        sliced_image = SlicedImage(
            image=image_tensor_slice, coco_image=coco_image, starting_pixel=[slice_bbox[0], slice_bbox[1]]
        )
        sliced_image_result.add_sliced_image(sliced_image)
